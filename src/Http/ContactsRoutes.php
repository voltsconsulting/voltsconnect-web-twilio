...
function handleContactsRoutes(string $uri, string $method, string $rootDir): bool
{
 ...
 $sql .= ' ORDER BY ct.id DESC LIMIT :lim OFFSET :off';
 ...
 $countSql = 'SELECT COUNT(*) as cnt FROM contacts ct WHERE 1=1';
 $countParams = [];
 if ($q !== '') {
 $countSql .= ' AND (ct.phone_number LIKE :q OR ct.name LIKE :q OR ct.first_name LIKE :q OR ct.last_name LIKE :q OR ct.email LIKE :q)';
 $countParams[':q'] = '%' . $q . '%';
 }
 if (!$isAdmin($pdo, $uid)) {
 $countSql .= ' AND EXISTS (
 ...
 json(['contacts' => $stmt->fetchAll(), 'limit' => $limit, 'offset' => $offset, 'total' => $total]);
 return true;
 }
